    ...
    pas_controls: {
        wfUpload: {
            render: function (control, _data) {
                ...
                uploadbtn.wijupload({
                    ...
                    action: "/Handler/upload.ashx?action=uploadfiles&uid=" + uid + "&tms=" + (new Date()).getTime()
                });
            ...
            _addfile: function (file, control, area) {
                ...
                if (file.FileID != null) {
                    link.attr("href", "/apps/pas/pages/home/downloadfile.aspx?FileNO=" + file.FileID);
                }


                if (control.attr("viewmodel") != "view") {
                    var cancel = $("<span style=\"width:5px;\" class='delete'>&nbsp;</span>").attr("title", "取消").appendTo(container);
        ...
        Upload: {
            render: function (control, _data) {
                ...
                var dirName = "";
                var dirPath = "";
                if (control.attr("filetype") != "" && typeof (control.attr("filetype")) != 'undefined') {
                    filetype = control.attr("filetype");
                }
                if (control.attr("dirName") != "" && typeof (control.attr("dirName")) != 'undefined') {
                    dirName = control.attr("dirName");
                }
                if (control.attr("dirPath") != "" && typeof (control.attr("dirPath")) != 'undefined') {
                    dirPath = control.attr("dirPath");
                }
                ...
                if (control.attr("filtermes") != "" && typeof (control.attr("filtermes")) != 'undefined') {
                    filter = $('<div  class="explain" style=\"color:#dddddd;font-size:12px;font-weight:bold;line-height:20px;\"><div/>').html(control.attr("filtermes"));
                } else {
                ...
                uploadContainer.wijupload({
                    ...
                    action: "/Handler/upload.ashx?action=uploadfiles&uid=-1&filetype=" + filetype + "&dirPath=" + dirPath + "&tms=" + (new Date()).getTime()
                });
                ...
                var btnWrap = function (flag) {
                    ...
                    if (flag) {
                        ...
                        jQuery('<div class="wisgrid-mask-msg-wrap"></div>')
			    .html("文件上传中,请稍等...")
			    .appendTo(wrap)
			    .css({
			        display: 'block',
			        left: (wrap.width() - jQuery('.wisgrid-mask-msg-wrap', wrap).outerWidth()) / 2,
			        top: (wrap.height() - jQuery('.wisgrid-mask-msg-wrap', wrap).outerHeight()) / 2
			    });
                    } else {
                        
                        jQuery('.wisgrid-mask', wrap).remove();
                        jQuery('.wisgrid-mask-msg-wrap', wrap).remove();
                    }
                ...
                var callback = function (newVal) {
                    ...
                    if (newVal) {
                        ...
                        if (data.FileID != null) {
                            link.attr("href", "/apps/pas/pages/home/downloadfile.aspx?FileNO=" + data.FileID);
                        }
                ...
                if (_data != null && typeof (_data) != 'undefined') {
                    ...
                    if (control.attr('IsPreView') == "true") {
                        $('#' + control.attr('PreViewId')).attr('src', "").hide();
                    }
                }
