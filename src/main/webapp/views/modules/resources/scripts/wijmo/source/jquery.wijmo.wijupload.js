...
(function ($) {
        ...
        var uploader,
			...
			Uploader = function () {
				...
				createXHR = function (name, action) {
				    ...
				    xhttpr.setRequestHeader("FileName", escape(name));
                ...
                try {
                    if (target.contentDocument) {
                        ...
                        for (i = 0; i < window.frames.length; i++) {
                            if (window.frames[i].name == iframe.attr("name")) {
    ...
    $.widget("wijmo.wijupload", {
        ...
        _createContainers: function () {
            ...
            /*修改：当只上传一个附件时不显示Upload All和Cancel All按钮， modified by xgz*/
            /*old:
            self._createCommandRow(commandRow);
            */
            //new
            if (self.options.maximumFiles != 1) {
                self._createCommandRow(commandRow);
            }
        },
        ...
        _createUploadButton: function () {
            ...
            if (self.options.uploadBtn) {
                addBtn = self.options.uploadBtn;
            }
            addBtn.mousemove(function (e) {
                //*xgz, var disabled = addBtn.data("button").options.disabled;
                var disabled = addBtn.prop("disabled");
                if (self.input) {
                    var pageX = e.pageX,
						    pageY = e.pageY;
...
_setOpacity: function (elem, l) {
        ...
        _createFileInput: function () {
            ...
            fileInput.attr("id", id)
				...
				.css("z-index", "9999")
				.css("opacity", 0)
				.height(addBtn.height() > 0 ? addBtn.height() : 15)
				.css("cursor", "pointer");
        ...
        _setAddBtnState: function () {
            ...
            if (files.length >= maxFiles) {
                //*xgz, addBtn.button({ disabled: true });
                ...
                //*xgz, addBtn.button({ disabled: false });
        ...
        _createFileRow: function (uploadFile) {
            ...
            //fileName = uploadFile.val(),
				...
				cancelBtn = $("<a>").attr("href", "javascript:void(0);") //修改：解决点击cancel按钮时，页面会自动滚动到页面头部的问题， modified by liukui
				.text("cancel")
        ...
        _createUploader: function (fileRow) {
            ...
            if (self._wijUpload()) {
                ...
                uploader.onComplete = function (obj) {
                    ...
                    //					}
                    //修改：增加obj参数,xgz
                    self._trigger("complete", obj.e, obj);
